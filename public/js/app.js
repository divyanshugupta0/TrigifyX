/* TrigifyX â€” frontend app (Firebase v9+ modular SDK)
 * Flow:
...
function renderProfile(p) {
  $("#disp-name").textContent = p.name || "â€”";
  $("#disp-email").textContent = p.email;
  $("#disp-tg").textContent = p.telegram || "â€”";
  ...
  // (No separate "linked" state â€” the chat id is editable any time.)
...
function renderSnippet(p) {
  ...
  // token are resolved server-side â€” never embedded in the page source.
...
function bindUI() {
  ...
  // Save the user's website URL (no connection check â€” just stored).
...
function showProfileComplete(u, p) {
  ...
  $("#complete-email").textContent = u.email || p.email || "â€”";
